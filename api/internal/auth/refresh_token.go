...
var (
	ErrTokenDoesNotBelongToCustomer = errors.New("token does not belong to Customer")
)
...
type RefreshTokenRepository interface {
	...
	RevokeByCustomerID(ctx context.Context, customerID uuid.UUID) error
...
func (r *refreshTokenRepository) RevokeByCustomerID(ctx context.Context, customerID uuid.UUID) error {
	now := time.Now()
	return r.db.WithContext(ctx).
		Model(&RefreshToken{}).
